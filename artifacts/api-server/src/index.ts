...
create table if not exists referrals (
  ...
  status text default 'pending', -- pending | milestone_1 | milestone_2 | complete
  ads_watched int default 0,
...
async function checkMilestones(user: any, newStreak: number) {
  ...
  const actualSundays = Math.floor(sundaysToBank * rewardMultiplier);

  // Update milestone attempts and banked sundays
  attempts[newStreak] = attemptCount + 1;

  const newBanked = (user.banked_sundays || 0) + actualSundays;
  const newBadges = [...(user.badges || [])];
  if (!newBadges.includes(newStreak)) newBadges.push(newStreak);
  ...
  await logStreakEvent(user.id, user.username, "milestone",
    user.current_streak, newStreak,
    user.banked_sundays, newBanked,
    {
      milestone: newStreak,
      sundaysGranted: actualSundays,
      attemptNumber: attemptCount + 1,
      rewardMultiplier,
    });

  // Bank future Sunday entries
  await bankSundayEntries(user, actualSundays, sunday);
}

async function bankSundayEntries(user: any, count: number, startingSunday: string) {
  const entries = [];
  let currentSunday = new Date(startingSunday);
...
app.post("/api/streak/heat", async (req, res) => {
  try {
    ...
    // Check free heat availability — 30 days from last use
    const lastHeatUsed = user.free_heats_reset_date;
    const thirtyDaysAgo = new Date(Date.now() - 30 * 86400000).toISOString();
    const heatAvailable = !lastHeatUsed || lastHeatUsed < thirtyDaysAgo;

    if (!heatAvailable) {
      const nextHeatDate = new Date(new Date(lastHeatUsed).getTime() + 30 * 86400000);
      const daysLeft = Math.ceil((nextHeatDate.getTime() - Date.now()) / 86400000);
      return res.status(402).json({
        error: "No free heats remaining",
        daysUntilNextFreeHeat: daysLeft,
        canPurchase: true,
        purchaseOptions: [
...
app.get("/api/streak/:username", async (req, res) => {
  try {
    ...
    const thirtyDaysAgo = new Date(Date.now() - 30 * 86400000).toISOString();
    const heatAvailable = !lastHeatUsed || lastHeatUsed < thirtyDaysAgo;
    const freeHeatsRemaining = heatAvailable ? 1 : 0;

    let daysUntilNextHeat = 0;
    if (!heatAvailable && lastHeatUsed) {
      const nextDate = new Date(new Date(lastHeatUsed).getTime() + 30 * 86400000);
      daysUntilNextHeat = Math.max(0, Math.ceil((nextDate.getTime() - Date.now()) / 86400000));
    }
