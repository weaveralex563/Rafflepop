...
app.post("/api/auth", async (req, res) => {
  try {
    ...
    if (referredBy) {
      ...
      if (referrer) {
        await supabase.from("referrals").insert({ referrer_username: referrer.id, referee_username: safeUsername, status: "pending" }).then(() => {}, () => {});
...
// adminAuth declared here so it can be used by all routes below
